...
async def tool_draw_diagram(doc, page: int, llm: GeminiClient) -> List[Dict[str, Any]]:
    ...
    # 1. 先生成 Image Prompt
    image_prompt = f"Create a technical diagram explaining this text: {text[:500]}"
    
    # 2. 调用生图 API
    image_bytes = await llm.generate_image(image_prompt)
    image_url = save_image_to_static(image_bytes)
    
    return [{
        "id": f"page-{page}-diagram",
        "type": "diagram",
        "title": f"Algorithm diagram for page {page}",
        "content": "Generated diagram based on page content.",
        "imageUrl": image_url,
        "page": page
    }]  "page": page
    }]
